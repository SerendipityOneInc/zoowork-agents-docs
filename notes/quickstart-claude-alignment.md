...
| 显式启动 Agent 并确认状态 | ZooWork 的 Session API 要求 `status.desired_state` 为 `running`。SDK 使用已有 `waitUntilRunning()`；curl 显示状态，必要时手动重复 GET。 |
...
| 后续 | 简化启动并等待的调用 | SDK 已有 `startAgent` 和 `waitUntilRunning`，可考虑组合入口。若要同时减少 curl 的状态检查，需公共 API 明确启动响应的就绪保证，单改 SDK 不能消除这一步。 |
...
- 在真实 PTY 中运行 Bash/curl 示例，覆盖状态重复查询、Ctrl+C 关闭事件流后保留 ID、继续清理，以及停止失败时不删除。

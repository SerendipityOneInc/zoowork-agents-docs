...
这个标签页会不会出现在你的设置页里，由两件互不相干的事决定：

- **你的角色。** 个人组织里任何成员都能建 key；企业组织里这个标签页要求 **admin** 角色。
- **你的账号在不在预览范围内。** App 会去问服务端：你的账号是不是跑在这套 API 所对应的
  agent runtime 上。答案不是「是」，标签页就不出现。Developer Preview 期间这是分批开放的，
  不是你自己能打开的开关。

两者中任何一个都可能是标签页不见的原因，兜底做法是同一个：**找你的组织管理员要一把 key。**
管理员建出来的和你自己建出来的是同一种凭证——它认证的是组织，不是创建它的那个人，所以一把 key
就够你们组织里所有要调这套 API 的服务用。

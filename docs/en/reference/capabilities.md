...
|---|---|---|
| Platforms you can bind | Verified | `feishu`, `slack`, `wecom` bind through `addChannel`; `feishu`, `wecom`, `weixin` bind through the QR flow. `weixin`/`wechat` on `addChannel` answer `400 channel.weixin_setup_required` — the QR flow it points at exists, so follow it. `discord`, `telegram`, `msteams`, `dingtalk-connector` and a mis-cased `WECOM` all answer `400 channel.invalid_request`. |
| `dingtalk` | Available, not verified | The name is in the channel service's platform list and binds with a `201`, unlike every other undocumented name, but no `config` for it has been made to work through this API. |
| Binding a stopped agent | Verified | No running-agent precondition here: an agent that has never been started accepts both paths. The channel comes online when the agent starts. |
...
| `addChannel()` | Verified | Answers `201` — but **credentials are not validated at bind time**. Bogus ones returned `201` with `health: 'unknown'` / `status: 'configured'`, then listed as `health: 'unhealthy'` / `status: 'error'`. Read the verdict from a follow-up list, never from the 201. `allow_from` in the body is accepted and ignored — the effective value is derived from `dm_policy`, and nothing echoes it back. Re-posting an identical body replays the same `201`, but the same `platform` + `account` with a **different** `config` answers `409 channel.conflict` — rotating credentials means remove-then-add. |
...
| Channel cleanup on `deleteAgent()` | Available, not verified | A successful delete best-effort unbinds the agent's channels — a removal, not a disable; a cleanup failure never turns the delete into an error. Confirmed only that the channels routes answer `404 service_api.not_found` once the agent is deleted. |

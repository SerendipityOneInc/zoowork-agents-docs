...
| **Source-reviewed, not verified** | Checked against code, not proof of deployment behavior, E2E or a published SDK. |
...
| `startAgent()` | Verified | Required for a stopped agent. Successful responses can contain informational warnings; this does not prevent HTTP/transport failures from throwing. |
| `stopAgent()` | Verified | Stops API session writes. Source-reviewed failure boundary: desired state can change before a later step fails. Read back before retrying an uncertain result. |
| Gate readiness on `status.desired_state` | Verified | The only correct readiness signal. Poll until it is `running`. |
| Gate readiness on `status.actual_state` | Not available | `actual_state` is a best-effort chat-channel health projection, not API readiness. Unsupported route-status can project `active` with zero channel counts and a health-unverified message; transient lookup failure can remain `activating`, and list/GET may briefly differ. `running` is not a member of its enum, so wait on `desired_state`. |
| `updateAgent()` | Verified | Omitted sections are preserved. Source-reviewed merge details: plain objects merge one level; nested values, arrays and scalars replace. tool_policy and system_prompt replace whole sections. |
...
| `deleteAgent()` | Verified | Soft deletion is not proof of schedule or sandbox cleanup. Stop first, explicitly remove owned schedules, and verify cleanup separately. |
...
| Agent list SDK pagination | Source-reviewed, not verified | The SDK change in [PR #26](https://github.com/SerendipityOneInc/zoowork-sdk-typescript/pull/26) preserves page metadata and supports `for await`, `hasNextPage()`, and `getNextPage()`. Covered by offline tests, not live verification. SDK 0.5.2 returns a one-page array; these helpers require a package release containing the change. See the [pagination reference](./typescript-sdk.md#listagentsopts). |
...
| `getSession()` | Verified + source-reviewed addition | Read run state from run_status, not status. Source-reviewed additions: nullable run_status plus optional numeric pending_approvals and pending_custom_tool_calls counts. |
...
| Legacy session listing under one agent | Available, not verified | `listSessions(agentId, { page })` and Python `list_sessions(page=...)` keep the fixed-50 numeric page lane, newest by updated_at. |
| Filtered cursor session listing | Source-reviewed, not verified | `listSessionPage()` / `list_session_page()` select the cursor lane (initial `sls1:0`), with limit 1–100, channel/surface/runtime/archive filters, per-row list_cursor and page next_cursor. Cursors are opaque and filter-scope-bound; invalid reuse returns `400 invalid_cursor`. |
...
| `user.custom_tool_result` | Source-reviewed, not verified | Returns an application-executed custom tool result by custom_tool_use_id or call_id. Supports text, JSON, and bounded base64 image blocks, is_error, and a stable idempotency_key. |
| Any other write-side event type | Not available | The write surface is exactly five types: `user.message`, `user.interrupt`, `user.tool_confirmation`, `user.custom_tool_result`, `system.message`. |
...
| `user.message.actor.ref` | Source-reviewed, not verified | Stable backend-selected attribution, including initial_events; not authentication or session/file isolation. IM sessions reject caller actor. |
| Resume a dropped stream | Verified | Resume with opaque ev.cursor in the cursor query. Source-reviewed gateway boundary: Last-Event-ID is not forwarded. after/seq selects the deprecated engine-only lane without inputs. Checkpoint after processing; application side effects are not exactly-once. |
...
| Two wire spellings for one event | Verified | Default unified REST and SSE both use snake_case. Legacy SSE can use camelCase. The SDK normalizes both; neither has top-level type. |
...
| Push delivery of events to your server | Not available | Use cursor-based SSE or listEventsPage and nextCursor. See [Not supported](./not-supported.md). |
...
|---|---|---|
| Platforms you can bind | Verified + source-reviewed addition | Live-verified: `feishu`, `slack`, `wecom` bind through `addChannel`; `feishu`, `wecom`, `weixin` bind through the QR flow. Source-reviewed only: `dingtalk-connector` binds directly with `clientId`/`clientSecret`, `dm_policy: 'open'`, and has no public guided-setup route. `weixin`/`wechat` on `addChannel` answer `400 channel.weixin_setup_required`. |
...
| `addChannel()` | Verified | Answers `201` — but **credentials are not validated at bind time**. Bogus ones returned `201` with `health: 'unknown'` / `status: 'configured'`, then listed as `health: 'unhealthy'` / `status: 'error'`. Read the verdict from a follow-up list, never from the 201. Source-reviewed: the public gateway ignores `allow_from`; it is not an ACL. Re-posting an identical body replays the same `201`, but the same `platform` + `account` with a **different** `config` answers `409 channel.conflict` — rotating credentials means remove-then-add. |
...
| Feishu document permission administration | Source-reviewed, not verified | Add, update and guided setup accept `permission_admin_enabled`. Responses may expose `capabilities.feishu_documents` with sync state, provider readiness, missing scopes, and `pending_admin` approval state. Enabling the request field is not proof that the capability is ready. |
...
| `tool_policy` allow and deny | Available, not verified | `{}` means the full manifest. Source-reviewed matching accepts an exact name, global `*`, or one trailing `prefix*` in allow/deny/rule match/afterRules and deferred MCP pinned entries; other `*` placements match nothing, and `alsoAllow` remains exact-only. We have not exercised a narrowed policy, so confirm it through `agent.tool`. |
| Application-executed custom tools | Source-reviewed, not verified | Declare up to 32 under `resource.custom_tools`; handle `agent.custom_tool_use`; resolve with `resolveCustomToolCall()` / `resolve_custom_tool_call()` or `user.custom_tool_result`. Pending recovery, timeout/cancel states, idempotent result events and the separate pending_custom_tool_calls count are covered by source and offline SDK tests. See [Tools](../build/tools.md#application-executed-custom-tools). |
| Remote HTTP MCP server | Verified + source-reviewed additions | Declared on the agent (`resource.mcp[]`), not as its own resource; transports are `streamable-http` (the default) and `sse`. Optional `exposure` is `deferred` (also the omission default) or `direct`; there is no `auto`. Deferred tools load through search/describe and remain available on later turns in the same Session; `direct` declares them from the first request (source-reviewed). Tools appear as `mcp__<server>__<tool>` — the server name must not contain an underscore — and execute against a public server. Healthy catalogs are config-bound. Source-reviewed failures can emit `mcp_connection_failed` or `mcp_authentication_failed` with optional reason; transient failed catalogs can expire and be probed on later resolution, not on a guaranteed recovery timer. It works **unauthenticated only**: the `credential` slug is accepted but the store behind it answers 404 through the gateway, so a server that needs auth cannot be made to work today. |
| MCP runtime context | Source-reviewed, not verified | `context.meta` adds `_meta["ai.zooclaw/context"]`; `context.headers` adds `x-zooclaw-*` headers. Both are opt-in. Context includes agent/session/computer identifiers and optional run/turn/config/actor fields. Catalog discovery carries no runtime context, and intermediaries may strip headers. |
| MCP permission defaults and overrides | Source-reviewed, not verified | `permission` sets `always_ask` or `always_allow` for the server. `tools` overrides exact native tool names, accepts no wildcards, and is capped at 64 entries. Omission defaults to allow. A Session-level allow-always decision on the server wildcard covers every tool from that server. |
| stdio MCP servers, MCP OAuth | Not available | Remote HTTP is the whole of it. |
| Approval-gated tool execution, end to end | Source-reviewed, not verified | REST methods and requested_at/decision fields exist; resolve may return 202/signaled while still pending. Deployment support, REST/event round trip and turn-budget handling remain unverified. Keep dangerous actions gated; unsupported deployments return 501. |
...
| Uploading your own skill | Source-reviewed, not verified | Root upload accepts org/personal; other scopes get public 400. Create description comes from ZIP frontmatter and options.description is ignored. Version upload allows the override and returns SkillVersionRecord (version/state). Duplicate-name create is 409; version uploads deduplicate content, not HTTP keys. |
...
| Create an environment and build a version | Source-reviewed, not verified | Configuration versions are immutable; createEnvironmentVersion creates a new one, not a retry. partial_ready can be transient or partial terminal. Query optional resourceClass (starter/pro/ultra); bound polling and choose an explicit readiness policy. |
...
| Application-executed tools | Source-reviewed, not verified | A waiting run can request a declared custom tool and receive a result from your process. This is not worker registration or a background queue. See [Tools](../build/tools.md#application-executed-custom-tools). |
| Worker registration and hosted queues | Not available | See [Not supported](./not-supported.md#worker-registration-and-hosted-queues). |

## Automation

| Capability | Status | Note |
|---|---|---|
| Schedules as an agent sub-resource | Available, not verified | List, create, replace, delete, trigger, and read runs, all under `/agents/{id}/schedules`, and all seven are on the client: `listSchedules`, `createSchedule`, `getSchedule`, `updateSchedule`, `deleteSchedule`, `triggerSchedule`, `listScheduleRuns`. Source-reviewed: interval schedules use everyMs and optional anchorMs; run records can include optional session_id. Change cadence through the input `schedule` vocabulary; the `scheduleSpec` a read hands back is refused by `ScheduleUpdate` and ignored by the server. And `triggerSchedule` against a disabled schedule answers `triggered: true` while the run projection records `status: 'skipped'`. |
